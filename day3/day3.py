...
class Beamline:
    ...
    def track_sigma_along(self, sigma0):
        '''
        return history of Sigma along the lattice to build beta(s)
        '''
        sigma=sigma0.copy()
        s_pos=0
        ...
        # for statistics
        s_history=[0]
        sigma_history=[sigma.copy()] # saving the whole matrix Sigma
    ...
    def extract_twiss(self, sigma, epsilon):
        '''Exatracts twiss parameters alpha beta gamma for Sigma'''
...
if __name__=="__main__":
    # SETTINGS
    EPSILON = 1E-6 # emittance typical for -e
    BETA0=10.0 # initial beta func [m]
    ALPHA0=0.0 # initial alpha (beam in waist)

    # creating init Sigma-matrix
    sigma0=make_sigma_from_twiss(BETA0,ALPHA0,EPSILON)
    print(f'initial emittance: {np.sqrt(np.linalg.det(sigma0)):.2e}')
    ...
    print(f"\n Start emittance: {emittance_list[0]:.2e}")
    print(f"End emittance:   {emittance_list[-1]:.2e}")
    print(f"Change:          {(emittance_list[-1] - emittance_list[0]) / emittance_list[0] * 100:.2e} %")
 
    # visualizing beta-function

    plt.figure(figsize=(10, 5))
    plt.plot(s, beta, 'b-', linewidth=2, label='β(s) [м]')
    plt.fill_between(s, 0, beta, alpha=0.3, color='blue')

    # adding markers of quadrupoles positions
    ...
    # === emittance visualization (has to be const) ===
